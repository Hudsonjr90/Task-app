...
const defaultTasks: Task[] = [
  {
    title: "Exemplo 1",
    important: false,
    description: "Descrição qualquer só para exemplo.",
    date: "2024-01-12",
    dir: "Principal",
    completed: true,
    id: "t1",
...
const getSavedDirectories = (): string[] => {
  ...
  if (localStorage.getItem("directories")) {
    ...
    const mainDirExists = dirList.some((dir: string) => dir === "Principal");
    if (!mainDirExists) {
      dirList.push("Principal");
    }
  } else {
    dirList.push("Principal");
  }
  ...
  if (localStorage.getItem("Tarefa")) {
    const savedTasksList = JSON.parse(localStorage.getItem("Tarefa")!);
...
const initialState: {
  ...
  tasks: localStorage.getItem("Tarefa")
