...
const ContentDirectories: React.FC<{ classActive: string }> = ({
  ...
  const createNewDirectoryHandler = (inputValue: string) => {
    ...
    if (newDirectoryName.length === 1) return;
  ...
  return (
    <>
      ...
      <button
        ...
        className="px-3 py-1 border-slate-300 dark:border-slate-700 border-2 ml-9 mt-2 rounded-md border-dashed hover:text-green-800"
      >
        + Novo
      </button>
    </>
