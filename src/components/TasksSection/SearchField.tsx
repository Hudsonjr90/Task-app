...
const ItemSearch: React.FC<{ task: Task }> = ({ task }) => {
  ...
  return (
    <li key={task.id} className="py-2">
      <Link
        to={`/task/${task.id}`}
        className="flex justify-between transition hover:text-red-500 dark:hover:text-slate-200"
      >
            ...
            {tasks.length ? (
              <>
                ...
                <button
                  ...
                  className="bg-red-100 w-full p-2 rounded-md text-green-800 dark:bg-slate-700/[.3] dark:text-slate-200"
                >
